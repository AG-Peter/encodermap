...
from .misc import distance_cost, pairwise_dist, periodic_distance, variable_summaries
...
class AngleDihedralCartesianEncoderMap(Autoencoder):
    ...
    def _prepare_data(self):
        ...
        assert (
            self.train_moldata.lengths.shape[1]
            == self.train_moldata.central_cartesians.shape[1] - 1
        )
        assert (
            self.train_moldata.angles.shape[1]
            == self.train_moldata.central_cartesians.shape[1] - 2
        )
        assert (
            self.train_moldata.dihedrals.shape[1]
            == self.train_moldata.central_cartesians.shape[1] - 3
        )
        self.train_data = (
            self.train_moldata.angles,
        ...
        if self.p.dihedral_cost_scale is not None:
            if self.p.dihedral_cost_variant == "mean_square":
                dihedral_cost = tf.reduce_mean(
                    tf.square(
                        periodic_distance(
                            self.inputs[1], self.generated_dihedrals, self.p.periodicity
                        )
                    )
                )
                ...
                raise ValueError(
                    "dihedral_cost_variant {} not available".format(
        ...
        if self.p.angle_cost_scale is not None:
            if self.p.angle_cost_variant == "mean_square":
                angle_cost = tf.reduce_mean(
                    tf.square(
                        periodic_distance(
                            self.inputs[0], self.generated_angles, self.p.periodicity
                        )
                    )
                )
                ...
                raise ValueError(
                    "angle_cost_variant {} not available".format(
                        self.p.auto_cost_variant
                    )
                )
    ...
    def generate(self, latent, quantity=None):
        if quantity is None:
            ...
            for batch in batches:
                angles, dihedrals, cartesians = self.sess.run(
                    (
                        self.generated_angles,
