...
class ManualPath(object):
    ...
    def _free_draw(self, event):
        self.lasso = Lasso(
            event.inaxes, (event.xdata, event.ydata), self._free_draw_callback
        )
    ...
    def _update_interp(self):
        ...
        x_i, y_i = self._interpolate(
            x_i, y_i
        )  # second iteration makes points more evenly spaced
    ...
    def _interpolate(self, x, y):
        ...
        interp_i = np.linspace(
            0, max(cumulative_distances), self.n_interpolation_points
        )
        try:
...
class PathGenerateCartesians(ManualPath):
    ...
    def use_points(self, points):
        ...
        self.mol_data.write(
            ...
            align_select=self.align_select,
        )
        if self.vmd_path:
            cmd = "{} {} {}".format(self.vmd_path, "generated.pdb", "generated.xtc")
            print(cmd)
            process = subprocess.Popen(
                cmd,
                cwd=current_save_path,
                shell=True,
...
class PathSelect(ManualPath):
    ...
    def use_points(self, points):
        ...
        self.mol_data.write(
            current_save_path,
            self.mol_data.cartesians[indices],
            only_central=False,
            align_reference=self.align_reference,
            align_select=self.align_select,
        )
