...
class Autoencoder:
    def __init__(
        ...
        """
        :param parameters: Parameters object as defined in :class:`encodermap.encodermap_tf1.parameters.Parameters`
            ...
            if not read_only:
                ...
                if self.validation_data is not None:
                    self.validation_writer = tf.summary.FileWriter(
                        os.path.join(self.p.main_path, "validation"), self.sess.graph
                    )
    ...
    def _prepare_data(self):
        if self.train_data is None:
            assert (
                self.n_inputs is not None
            ), "If no train_data is given, n_inputs needs to be given"
        ...
        elif isinstance(self.train_data, TFRecordDataset):
            pass
        else:
            raise ValueError(
    ...
    def _setup_data_iterator(self):
        self.data_placeholders = tuple(
            tf.placeholder(dat.dtype, dat.shape) for dat in self.train_data
        )
        ...
        with tf.name_scope("encoder"):
            ...
            for i, (n_neurons, act_fun) in enumerate(
                ...
                if act_fun:
                    ...
                    act_fun = None
                variable_summaries("activation{}".format(i), current_layer, debug=True)
    ...
    def _generate(self, inputs):
        with tf.name_scope("generator"):
            ...
            for n_neurons, act_fun in zip(
                n_neurons_with_inputs[-2::-1], self.p.activation_functions[-2::-1]
            ):
                if act_fun:
                    act_fun = getattr(tf.nn, act_fun)
                else:
        ...
        if self.p.auto_cost_scale is not None:
            if self.p.auto_cost_variant == "mean_square":
                auto_cost = tf.reduce_mean(
                    tf.square(
                        periodic_distance(
                            self.main_inputs, self.generated, self.p.periodicity
                        )
                    )
                )
                ...
                raise ValueError(
                    "auto_cost_variant {} not available".format(
                        self.p.auto_cost_variant
                    )
                )
    ...
    def train(self):
        ...
        for i in tqdm(range(self.p.n_steps)):
            ...
            if self.seed is not None:
                ...
                x = tf.train.get_global_step(self.graph)
                x = x.eval(session=self.sess)

                # read the current Summary
                values = {}
                values["global_step"] = x
                summary_proto = tf.Summary().FromString(summary_values)
                for entry in summary_proto.value:
                    if "histogram" in entry.tag:
                        values[entry.tag] = entry.histo
                    else:
                        values[entry.tag] = entry.simple_value
                return values
            ...
            self.saver.save(
                self.sess,
                os.path.join(
                    self.p.main_path, "checkpoints", "step{}.ckpt".format(self._step())
                ),
            )
    ...
    def profile(self):
        ...
        for i in range(5):
            ...
            with open(
                os.path.join(self.p.main_path, "timeline{}.json".format(i)), "w"
            ) as f:
    ...
    def close(self):
        ...
        if "sess" in self.__dict__.keys():
            self.sess.close()
        try:
            tf_ops.dismantle_graph(
                self.graph
            )  # not implemented in older versions of tensorflow
