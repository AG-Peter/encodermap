...
def add_layer_summaries(layer, debug=False):
    """
    ...
    variable_summaries(layer.name + "_weights", weights, debug)
    variable_summaries(layer.name + "_biases", biases, debug)
...
def variable_summaries(name, variables, debug=False):
    """
    ...
    for i, var in enumerate(variables):
        ...
        with tf.name_scope(name):
            ...
            tf.summary.tensor_summary("values", var)
...
def search_and_replace(
    file_path, search_pattern, replacement, out_path=None, backup=True
):
    """
...
def random_on_cube_edges(n_points, sigma=0):
    ...
    a = np.array([[0, 0, 0]] * 3 + [[x, y, 0]] * 3 + [[0, y, z]] * 3 + [[x, 0, z]] * 3)
    ...
    for i in range(12):
        ...
        coordinates[mask] += (
            a[i] + (np.expand_dims(r[mask], axis=1) - i / 12) * 12 * b[i]
        )
        ids[mask] = i
...
def potential_energy(angles, dihedrals, distances):
    ...
    energies = tf.where(
        distances < 2,
        tf.minimum(10 - distances, (1 / distances) ** 12),
        tf.zeros(tf.shape(distances)),
    )
