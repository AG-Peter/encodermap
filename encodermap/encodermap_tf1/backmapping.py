...
def dihedrals_to_cartesian_tf_old(
    ...
    if len(cartesian.get_shape()) == 2:
        cartesian = tf.tile(
            tf.expand_dims(cartesian, axis=0), [tf.shape(dihedrals)[0], 1, 1]
        )
    ...
    for i in range(n):
        ...
        rotated = cartesian[:, cai[j + 2] : cai[j + 2] + 1] + tf.matmul(
            cartesian[:, cai[j + 2] + 1 :] - cartesian[:, cai[j + 2] : cai[j + 2] + 1],
...
def dihedrals_to_cartesian_tf(dihedrals, cartesian):
    ...
    new_cartesian_right = dihedral_to_cartesian_tf_one_way(
        dihedrals_right, cartesian_right
    )
...
def merge_cartesians(
    central_cartesians, central_atom_names, H_cartesians, O_cartesians
):
